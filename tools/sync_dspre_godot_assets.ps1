...
$actualSourceGlbFiles = @(Get-ChildItem -LiteralPath $SourceRoot -Recurse -Filter "*.glb" -File)
...
$sourcePngFiles = @(Get-ChildItem -LiteralPath $sourceTextureRoot -Filter "*.png" -File)
$allSourcePngFiles = @(Get-ChildItem -LiteralPath $SourceRoot -Recurse -Filter "*.png" -File)
...
foreach ($material in $catalogMaterials) {
    ...
    if ($materialKey -notmatch '^mat_[0-9a-f]{64}$' -or -not $catalogMaterialKeys.Add($materialKey)) {
