...
func _init() -> void:
	...
	_assert(int(stats.get("source_font_role_atlas_count", 0)) == 11, "unexpected source font role atlas count")
	_assert(int(stats.get("source_font_atlas_binding_count", 0)) == 12, "unexpected source font atlas binding count")
	_assert(int(stats.get("render_text_material_count", 0)) == 16, "unexpected RenderText material count")
	_assert(int(stats.get("glyph_encoding_single_byte_count", 0)) > 100, "unexpected charmap single-byte count")
	...
	var move_desc_text := _dict(move_desc.get("text_info", {}))
	var move_desc_indices := _dict(move_desc_text.get("text_color_indices", {}))
	_assert(int(move_desc_indices.get("foreground", 0)) == 13, "expected resolved move description dynamic foreground")
	_assert(int(move_desc_indices.get("background", 0)) == 14, "expected resolved move description dynamic background")
	_assert(int(move_desc_indices.get("shadow", 0)) == 15, "expected resolved move description dynamic shadow")
	...
	_assert(String(normal_atlas.get("role_mask_status", "")) == "source_font_role_mask", "expected normal source font role mask binding")
	_assert(_asset_exists({"image": normal_atlas.get("latin_image", "")}), "expected normal Latin source font atlas PNG")
	_assert(_asset_exists({"image": normal_atlas.get("latin_role_mask_image", "")}), "expected normal Latin source font role mask PNG")
	...
	_assert(String(latin_normal.get("role_mask_status", "")) == "generated_source_font_role_mask", "expected Latin normal role mask")
	_assert(_asset_exists({"image": latin_normal.get("role_mask_image", "")}), "expected Latin normal role mask asset")
	var role_counts := _dict(latin_normal.get("role_mask_role_counts", {}))
	_assert(int(role_counts.get("1", 0)) > 0 and int(role_counts.get("2", 0)) > 0, "expected visible role pixels in Latin normal role mask")
	var render_text_materials := _dict(data.get("render_text_materials", {}))
	var material_colors := _dict(render_text_materials.get("colors", {}))
	var material_13 := _dict(material_colors.get("13", {}))
	_assert(String(render_text_materials.get("status", "")) == "generated_from_textbox_indexed_colors", "expected generated RenderText material table")
	_assert(int(render_text_materials.get("material_count", 0)) == 16, "expected 16 RenderText material entries")
	_assert(int(_dict(material_colors.get("0", {})).get("a", -1)) == 0, "expected transparent material zero")
	_assert(int(material_13.get("r", -1)) == 213 and int(material_13.get("g", -1)) == 74, "expected textbox material 13 color")
	var glyph_encoding := _dict(text_printer.get("glyph_encoding", {}))
	var single_byte_chars := _dict(glyph_encoding.get("single_byte_chars", {}))
	_assert(String(glyph_encoding.get("status", "")) == "generated_from_charmap", "expected generated charmap metadata")
	_assert(int(single_byte_chars.get("F", 0)) == 0xC0, "expected charmap F byte")
	_assert(int(single_byte_chars.get("P", 0)) == 0xCA, "expected charmap P byte")
	...
	print(JSON.stringify({
		...
		"source_font_role_atlas_count": int(stats.get("source_font_role_atlas_count", 0)),
		"render_text_material_count": int(stats.get("render_text_material_count", 0)),
