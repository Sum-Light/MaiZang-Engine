...
func _check_pokemon_assets(data: Dictionary) -> void:
	...
	_assert(int(stats.get("missing_palette_asset_count", -1)) == 0, "expected Pokemon palette metadata to resolve")
	...
	for species_symbol in ["SPECIES_TORCHIC", "SPECIES_GEODUDE", "SPECIES_PIKACHU"]:
		var record := _dict(sprites.get(species_symbol, {}))
		_assert(_pokemon_palette_color_count(record, "normal") == 16, "expected normal palette for %s" % species_symbol)
		_assert(_pokemon_palette_color_count(record, "shiny") == 16, "expected shiny palette for %s" % species_symbol)
		_palette_metadata_count += 2
...
func _check_trainer_assets(data: Dictionary) -> void:
	...
	_assert(int(stats.get("front_palette_metadata_count", 0)) == 155, "unexpected trainer front palette count")
	_assert(int(stats.get("back_palette_metadata_count", 0)) == 10, "unexpected trainer back palette count")
	...
	for pic_symbol in _array(data.get("front_sprite_order", [])):
		...
		_assert(_palette_color_count(_dict(record.get("palette", {}))) == 16, "expected trainer front palette for %s" % pic_symbol)
		_palette_metadata_count += 1
	for pic_symbol in _array(data.get("back_sprite_order", [])):
		...
		if _check_sprite_metadata(_dict(record.get("back", {})), "%s.back" % pic_symbol, false):
			trainer_image_refs += 1
		_assert(_palette_color_count(_dict(record.get("palette", {}))) == 16, "expected trainer back palette for %s" % pic_symbol)
		_palette_metadata_count += 1
...
func _check_environment_assets(data: Dictionary) -> void:
	...
	for environment_symbol in _array(data.get("environment_order", [])):
		...
		_assert(_palette_color_count(_dict(record.get("palette", {}))) > 0, "expected battle environment palette for %s" % environment_symbol)
...
func _check_transition_assets(data: Dictionary) -> void:
	...
	_assert(int(stats.get("source_palette_asset_count", 0)) == 19, "unexpected battle transition palette count")
	...
	for asset_id in _array(data.get("asset_order", [])):
		...
		if kind == "texture":
			...
			_assert(_palette_color_count(asset) > 0, "expected transition palette metadata for %s" % asset_id)
			_palette_metadata_count += 1
	...
	_assert(int(_dict(assets.get("graphics/battle_transitions/big_pokeball.png", {})).get("source_palette_color_count", 0)) == 16, "expected Big Pokeball transition palette conversion metadata")
	_check_image_asset(_dict(assets.get("graphics/battle_transitions/team_aqua.png", {})), "Aqua transition texture", ALPHA_TRANSPARENT)
	_assert(int(_dict(assets.get("graphics/battle_transitions/team_aqua.png", {})).get("source_palette_color_count", 0)) == 16, "expected Aqua transition palette conversion metadata")
...
func _check_sprite_metadata(asset: Dictionary, label: String, has_frame_size: bool) -> bool:
	...
	_assert(int(transparency.get("source_palette_index", -1)) == 0, "expected palette index 0 alpha metadata for %s" % label)
	if has_frame_size:
...
func _pokemon_palette_color_count(record: Dictionary, palette_key: String) -> int:
	return _palette_color_count(_dict(_dict(record.get("palettes", {})).get(palette_key, {})))


func _palette_color_count(palette: Dictionary) -> int:
	var value = palette.get("color_count", 0)
	if value == null:
		return 0
	return int(value)
