...
func _init() -> void:
	...
	_assert(_array(sawyer.get("tests", [])).has("tools/godot_smoke/battle_asset_image_quality_smoke.gd"), "expected trainer asset image quality smoke reference")
	...
	_assert(_array(geodude.get("tests", [])).has("tools/godot_smoke/battle_asset_image_quality_smoke.gd"), "expected Pokemon asset image quality smoke reference")
	...
	_assert(_array(grass_environment.get("tests", [])).has("tools/godot_smoke/battle_asset_image_quality_smoke.gd"), "expected battle environment asset image quality smoke reference")
	...
	_assert(String(big_pokeball_transition.get("tilemap_status", "")) == "first_pass", "expected Big Pokeball first-pass tilemap composite")
	_assert(int(big_pokeball_transition.get("tilemap_composite_count", 0)) == 1, "expected Big Pokeball tilemap composite count")
	_assert(String(big_pokeball_transition.get("first_tilemap_composite", "")).ends_with("assets/generated/battle_transitions/composites/big_pokeball_map.png"), "expected Big Pokeball composite path")
	...
	_assert(_array(big_pokeball_transition.get("tests", [])).has("tools/godot_smoke/battle_asset_image_quality_smoke.gd"), "expected battle transition asset image quality smoke reference")
	...
	_assert(_array(textbox_interface.get("tests", [])).has("tools/godot_smoke/battle_scene_source_profile_smoke.gd"), "expected battle scene source profile smoke reference")
	...
	_assert(_array(textbox_interface.get("unsupported", [])).has("battle_window_source_capture_pending"), "expected textbox source capture pending")
	_assert(String(textbox_interface.get("source_capture_status", "")) == "missing_source_captures", "expected source capture status to be explicit")
	_assert(int(textbox_interface.get("expected_source_capture_count", 0)) == 3, "expected three battle window source captures")
	_assert(int(textbox_interface.get("missing_source_capture_count", 0)) == 3, "expected three missing battle window source captures")
	_assert(_array(textbox_interface.get("source_capture_paths", [])).has("assets/source/battle_window_captures/action.png"), "expected action source capture path")
	_assert(String(textbox_interface.get("source_scene_capture_profile", "")).ends_with("assets/source/battle_scene_captures/emerald_2011_zh"), "expected full-scene source profile path")
	_assert(int(textbox_interface.get("expected_source_scene_capture_count", 0)) == 24, "expected 24 source scene profile captures")
	_assert(int(textbox_interface.get("missing_source_scene_capture_count", -1)) == 0, "expected source scene profile captures to be present")
	...
	_assert(_registry_has_code(unsupported_registry, "battle_window_source_capture_pending"), "expected battle window source capture unsupported registry code")
