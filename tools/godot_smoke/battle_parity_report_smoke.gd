...
func _init() -> void:
	...
	_assert(_array(quick_wild.get("unsupported", [])).has("debug_launcher_pending"), "expected quick wild unsupported code")
	_assert(_array(trainer_selector.get("unsupported", [])).has("debug_launcher_pending"), "expected trainer launcher unsupported code")
