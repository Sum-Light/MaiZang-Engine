...
- `Main` connects the debug world, player, camera, HUD status label, debug dialogue panel, and first-pass transition overlay, and shows whether map data came from generated JSON or fallback constants.
...
- Explicit-position and generated warp-id transition effects are now applied when generated destination data exists: `EventManager` uses `DataRegistry` to load destination map/tileset/script data, reconfigures `MapRuntime`, updates `GameState.current_map_id`, and moves the player either to the explicit destination coordinate or to `events.warp_events[warp_id]` in the destination map. Before applying the map change, it emits a `transition_sequence_requested` dictionary with source-traced presentation steps.
- Transition sequence data currently records a `60fps` frame basis and source references for `DoWarp`, `DoDoorWarp`, `Task_DoDoorWarp`, `FieldCB_DefaultWarpExit`, `SetUpWarpExitTask`, metatile behavior helpers, door animation tables, and normal tile-walk timing. Door sequences include source-order lock/freeze, door sound intent, 16-frame open, 16-frame player step up, hide player, 16-frame close, fade/load/fade, source-derived exit-task selection, conditional exit-door step down, and unlock steps. Normal/silent sequences record lock, fade/load/fade, source-derived exit-task selection, and unlock steps.
- `Main` currently consumes transition sequences with a black overlay and input lock whose duration sums recorded `duration_frames`. This is a scaffold consumer, not final source-faithful playback. Real door metatile animation, audio playback, exact fade color selection, actual destination exit-task animation playback, save callbacks, destination map scripts, and player visibility remain future work.
