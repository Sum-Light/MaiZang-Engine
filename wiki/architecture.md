...
- `ObjectEventSpawner` draws generated object events through lightweight placeholder nodes. The current first imported sprite path consumes source `OBJ_EVENT_GFX_BOY_1` static facing frames and hFlip metadata; full object-event animation task playback remains future work.
...
- Field-to-trainer-battle handoff goes through `EventManager.request_trainer_battle_start`; map/event code sends a structured request and does not call `BattleEngine` directly. The bridge rejects missing trainer ids, only permits empty-party fallback for explicit debug requests, creates that fallback as temporary non-persistent party data, and rejects non-single trainer battle states until a source-backed double/partner battle presentation exists.
