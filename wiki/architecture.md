...
- `EventManager.try_dispatch_standard_wild_encounter` follows the traced `CheckStandardWildEncounter` state shape with four immunity steps, previous metatile behavior, new-metatile 60% gate via `EncounterEngine`, and debug/request output. It is still a request/debug path, not battle setup.
- `EncounterEngine`/`EventManager` do not yet implement ability-influenced species slot selection, roamer/mass-outbreak/double-battle branches, full Battle Pike/Pyramid generation, Route 119 Feebas, Sweet Scent, fishing bite/minigame timing, full avatar/weather/flute item-use presentation integration, Synchronize/gender/nature/personality/IV creation, enemy party creation, battle transitions, Safari Pokeblock behavior, full DexNav hidden encounters, or Sootopolis legendary suppression. Those must remain visible as unsupported/future notes until traced runtime and presentation slices are implemented.
