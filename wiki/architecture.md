...
- `ScriptVM` message execution now preserves both expanded `text` and source `unexpanded_text`, plus per-occurrence `placeholder_substitutions`. Runtime placeholder expansion currently covers `{STR_VAR_1}`, `{STR_VAR_2}`, and `{STR_VAR_3}` from VM string vars; other source placeholders such as `{PLAYER}`, `{KUN}`, and rival-name placeholders remain future traced text/runtime work.
