...
- `TransitionSequencePlayer` consumes source-traced transition sequence data for the current first-pass presentation: fade overlay, input lock, player visibility, scripted player steps, deferred map load application, and final door-exit player position updates.
...
- `TransitionSequencePlayer` currently consumes transition sequences with a black overlay, input lock, player visibility toggles, and scripted player movement. `Task_ExitDoor` playback now leaves the player one tile below the destination door, matching the source visible exit movement. Real door metatile animation, audio playback, exact fade color selection, non-animated door/stair exit playback, save callbacks, destination map scripts, and full player visibility semantics remain future work.
