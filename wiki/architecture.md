...
- `EventManager` can run source map header scripts by type through generated `map_script_table` records. The current first pass supports automatic `MAP_SCRIPT_ON_LOAD` during initial load and transition load, runs the target script through `ScriptVM`, and applies supported runtime effects through `MapRuntime`.
