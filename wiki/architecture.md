...
- `EventManager.try_dispatch_standard_wild_encounter` follows the traced `CheckStandardWildEncounter` state shape with four immunity steps, previous metatile behavior, new-metatile 60% gate via `EncounterEngine`, and debug/request output. It now calls `BattleEngine.create_wild_battle_state` for standard wild hits, using `PartyRuntime.build_battle_party` when available and first-live-player selection metadata to match the source send-out path.
