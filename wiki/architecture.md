...
- Transition sequence data currently records a `60fps` frame basis and source references for `DoWarp`, `DoDoorWarp`, `Task_DoDoorWarp`, `FieldCB_DefaultWarpExit`, door animation tables, and normal tile-walk timing. Door sequences include source-order lock/freeze, door sound intent, 16-frame open, 16-frame player step up, hide player, 16-frame close, fade/load/fade, exit-task selection, conditional exit-door step down, and unlock steps. Normal/silent sequences record lock, fade/load/fade, exit-task selection, and unlock steps.
- `Main` currently consumes transition sequences with a black overlay and input lock whose duration sums recorded `duration_frames`. This is a scaffold consumer, not final source-faithful playback. Real door metatile animation, audio playback, exact fade color selection, destination exit-task branching from metatile behavior names, save callbacks, destination map scripts, and player visibility remain future work.
