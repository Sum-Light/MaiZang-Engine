...
- Wild encounter runtime behavior is not implemented by the generated data alone. Step timing, encounter-rate rolls, Repel/Lure encounter modifiers, ability modifiers, surfing/fishing flags, Sweet Scent, DexNav, mass outbreaks, Feebas tiles, roamers, Battle Pike/Pyramid behavior, battle setup, audio, and presentation timing must still be implemented only after tracing the corresponding source C and referenced resources.
