...
- `EventManager` owns the first field-step standard wild encounter dispatch state: source-style four-step immunity, previous/current metatile behavior tracking, `OW_FLAG_NO_ENCOUNTER`-style blocking, and a structured encounter request when `EncounterEngine` returns a candidate. It resets immunity on generated map transitions. It does not yet create enemy parties or start battle presentation.
...
- Coordinate-event execution is currently dispatched after player tile movement in `Main`, followed by first-pass generated current-cell warp-event dispatch when no coordinate event matched, then first-pass standard wild encounter dispatch when neither consumed the step. Blocked front-cell door warp dispatch only fires while facing north, matching source `TryDoorWarp`. Misc walking scripts, step-count scripts, Repel/DexNav step handling, weather presentation, and forced-movement script chaining remain future work.
...
- `EventManager.try_dispatch_standard_wild_encounter` is the first field-step consumer. It follows the traced `CheckStandardWildEncounter` state shape with four immunity steps, previous metatile behavior, new-metatile 60% gate via `EncounterEngine`, and debug/request output after generated coord events and current-cell step warps. It explicitly reports the not-yet-integrated source ordering around misc walking scripts, step-count scripts, Repel counter presentation, DexNav, walk-into-signpost checks, arrow warps, and front-cell door warp ordering.
- `EncounterEngine`/`EventManager` do not yet implement Repel/lure/ability/flute/Cleanse Tag modifiers, roamer/mass-outbreak/double-battle branches, Battle Pike/Pyramid generation, Route 119 Feebas, Sweet Scent, fishing bite/minigame timing, Synchronize/gender/nature/personality/IV creation, enemy party creation, battle transitions, Safari Pokeblock behavior, DexNav hidden encounters, or Sootopolis legendary suppression. Those must remain visible as unsupported/future notes until traced runtime and presentation slices are implemented.
