...
- `SaveService` is registered as a domain/runtime autoload. Its first slice builds source-traced Godot JSON save snapshots from `GameState`, writes and reads them under `user://`, restores player profile/location/flags/vars/party data, records save status/counter metadata, and exposes source save-flow labels without depending on save menu presentation.
...
- Async save task timing, save window layout, text printer waits, YES/NO menu task, `SE_SAVE`/`SE_BOO`, object-event save/load, bag/mail/options, Pokemon storage, special sectors, Hall of Fame, link save variants, save failure screen, and full continue-warp behavior remain future source-traced systems.
