...
- Wild encounter runtime behavior is not implemented by the generated data alone. Step timing, encounter-rate rolls, Repel/Lure encounter modifiers, ability modifiers, surfing/fishing flags, Sweet Scent, DexNav, mass outbreaks, Feebas tiles, roamers, Battle Pike/Pyramid behavior, battle setup, audio, and presentation timing must still be implemented only after tracing the corresponding source C and referenced resources; the first Repel/Lure generation slice is now traced and implemented for standard encounters.
- `EncounterEngine` is the first runtime consumer for generated wild encounter data. It implements map-record lookup, Altering Cave table selection through `VAR_ALTERING_CAVE_WILD_SET`, source `MB_*` metatile behavior routing for standard land/water encounters, land/water/rock-smash/fishing table lookup, source slot probability selection, fishing rod group selection, first-pass level selection, `WildEncounterCheck`-style `encounterRate * 16` metadata, Lure encounter-rate doubling with cap, Lure 20% slot reversal and same-species max-level +1 behavior, and Repel level filtering after a wild level is selected.
