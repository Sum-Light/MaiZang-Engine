...
- `ScriptVM` message execution now preserves both expanded `text` and source `unexpanded_text`, plus per-occurrence `placeholder_substitutions` with source placeholder ids. Runtime placeholder expansion currently covers `{PLAYER}` from `GameState.player_name`, `{KUN}` as the current source's empty honorific placeholder, and `{STR_VAR_1}`, `{STR_VAR_2}`, and `{STR_VAR_3}` from VM string vars. Rival-name, version, team, and other source placeholders remain future traced text/runtime work.
