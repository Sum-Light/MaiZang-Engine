...
- Trainer party construction reads generated trainer records through `DataRegistry` and follows the first-pass source shape of `CreateNPCTrainerPartyFromTrainer`: party order is preserved, explicit moves are assigned with `GetMovePP`-equivalent PP from generated move data, held item/ability/nature/friendship metadata is carried forward, and source default level-up move assignment reports `missing_level_up_learnset_data` until learnsets are generated.
