...
Sprite depth runtime update: `LayerAwareMapRenderer` now splits its draw path so the parent pass draws bottom/middle layer roles and a `TopLayerOverlay` child draws the BG1/top role above the default sprite band. `scripts/overworld/overworld_depth.gd` preserves the source `src/overworld.c:sOverworldBgTemplates` BG priorities plus `src/event_object_movement.c:sElevationToPriority`/`sElevationToSubpriority`; `PlayerController` and `ObjectEventPlaceholder` set Godot z-index records from that contract, putting default first-slice elevation-3 sprites between BG2/middle and BG1/top. Runtime status is `normal_covered_split_object_depth_first_pass`; `Main` installs `LayerAwareMapRenderer` at runtime while keeping the existing `DebugMapPlane` node as a hidden fallback API target. `layer_aware_map_renderer_smoke`, `object_event_sprite_smoke`, `player_turn_input_smoke`, and `main_debug_battle_smoke` verify top-overlay z order, default object/player z-index records, and main-scene integration. Exact camera-offset OAM subpriority, bridge subsprites, shadows, reflections, movement-task priority updates, door forced-covered redraws, and per-cell redraw caches remain pending.
