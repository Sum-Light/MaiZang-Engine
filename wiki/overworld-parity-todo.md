...
- [x] Add tests for animated water, flowers, currents, lava, falls, sand/water edges, and any first-slice General/Petalburg animations.
- [x] Add unsupported metadata for callbacks not yet rendered even if their source tilesets import successfully.
...
Animation smoke coverage update: `tools/godot_smoke/tileset_animation_player_smoke.gd` now verifies source-timed `TilesetAnim_General` cadence across frame 1 water, frame 2 sand/water edge, frame 3 waterfall, frame 4 land/water edge, and frame 16 flower, including RGBA frame-strip selection, tile-copy destination ranges, renderer atlas patching where the current first-slice atlas has visible metatile slots, and safe renderer idle reporting where no current-map slot matches. The same smoke covers first-slice `gTileset_Petalburg` as an init-only secondary callback with `NULL`, Mauville VDest flower requests, Underwater seaweed, and metadata-only no-renderer scheduling for Pacifidlog water currents and Lavaridge lava until those maps have generated runtime layer atlases. `layer_aware_map_renderer_smoke.gd` still passes, and runtime `scripts/`/`scenes/` still have 0 `palette`/`source_color`/`source_palette` references. Section 6 progress is now 13/14 complete; total overworld checklist progress is 76/296.

Unsupported callback metadata update: `TilesetAnimationPlayer` frame snapshots now expose `unsupported_count` plus per-callback `unsupported` entries whenever a source-scheduled tileset animation is imported and triggered but not rendered into the current runtime atlas. The metadata distinguishes no configured layer renderer (`tileset_animation_callback_metadata_only_renderer_not_configured`), a configured renderer without generated layer-atlas data for the map (`tileset_animation_callback_metadata_only_missing_renderer_data`), and emitted tile-copy requests with no patchable current-map slot (`tileset_animation_callback_not_rendered`). Each entry records queue function, role, tileset, event callback symbol, counter/timer values, renderer request status, source trace, and `source_equivalent_for_runtime_rendering = false` while preserving `rebuilds_full_map = false` and no map-data/collision/elevation/generated-file mutation. Smoke coverage now verifies applied Littleroot General water, sand/water edge, waterfall, and flower callbacks have no unsupported marker, Littleroot land/water edge reports `no_matching_metatile_slots`, missing renderer-data setup is explicit, and Pacifidlog/Lavaridge no-renderer paths remain metadata-only. Runtime `scripts`/`scenes` still have 0 `palette`/`source_color`/`source_palette` references. Section 6 is now complete at 14/14; total overworld checklist progress is 77/296.
