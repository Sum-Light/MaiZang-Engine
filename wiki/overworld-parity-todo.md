...
Layer debug view update: `LayerAwareMapRenderer` now exposes `set_layer_debug_view`, `cycle_layer_debug_view`, and `get_layer_debug_view_status` for `all`, `bottom`, `middle`, and `top` presentation-only modes. `Main` binds F8 to cycle the mode and reports the current value in the status label. The debug status explicitly reports no gameplay, map-data, collision, or elevation mutation; smoke coverage verifies that source map block ids and tileset metatile entries remain unchanged while top-layer visibility toggles. Section 5 progress is now 8/12 complete; total overworld checklist progress is 64/296.
