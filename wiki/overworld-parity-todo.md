...
Callback-to-map metadata update: `tileset_header_report.json` now includes `tileset_callback_map_report`, linking source `struct Tileset.callback` bindings to primary/secondary layout usage and map source records. It covers 785 layouts, 939 map source records, 137 layout tileset pairs, 136 referenced tilesets, 31 callback symbols, and 4 ungrouped unused map folders, while preserving the known missing secondary header `0` for `gTileset_General+0`. Representative first-slice rows show `InitTilesetAnim_General` on 238 layouts/227 map records and `InitTilesetAnim_Petalburg` on 6 layouts/6 map records, both including `MAP_LITTLEROOT_TOWN`. Section 6 progress is now 6/14 complete; total overworld checklist progress is 69/296.
