...
- [ ] B8.2 Implement source window layer renderer.
  ...
  - Current generated/runtime metrics: 10 source-window composite rects are generated from `sStandardBattleWindowTemplates`; 25 normal battle-window `text_info` records are generated from `sTextOnWindowsInfo_Normal`; `text_printer` metadata preserves `BattlePutTextOnWindow` message speed source, A/B speed-up flag intent, recorded-battle speeds `[8,4,1,0]`, and player text speed delay/modifier/scroll tables. Runtime `BattleTextPrinter` resolves `GetPlayerTextSpeedDelay`, models the source `AddTextPrinter` nonzero-speed `speed - 1` delay counter, supports speed-zero synchronous menu text, can parse generated `source_text` records for `\n`, `\l`, `\p`, trailing `$`, `{PAUSE n}`, `{PAUSE_UNTIL_PRESS}`, metadata-only audio tokens such as `{PLAY_SE ...}`/`{WAIT_SE}`, and style controls, and still falls back to display-text events when no source text is available. It now also propagates generated `encoding.bytes`/hex when text records have no runtime substitutions, drives first-pass events directly from source bytes for `CHAR_NEWLINE`, `CHAR_PROMPT_SCROLL`, `CHAR_PROMPT_CLEAR`, `PLACEHOLDER_BEGIN`, `CHAR_DYNAMIC`, `CHAR_KEYPAD_ICON`, `CHAR_EXTRA_SYMBOL`, and known EXT control codes, reports byte-control summaries with offsets/args, and splits prompt-scroll versus prompt-clear waits in runtime snapshots. Visible glyph text in source-byte mode is still a temporary fallback from `source_text`/display text, not a decoded source font renderer.
  - Remaining for this task: exact source glyph renderer, full charmap byte-to-glyph decoding/layout, full `RenderText` control-code pixel/layout side effects, exact scroll/clear window pixel behavior, full link/recorded battle context, and screenshot comparison against source captures.
  - Current validation: `tools/godot_smoke/data_registry_battle_interface_smoke.gd` verifies the 10 generated composite rects plus 25 generated text-info records and text-printer metadata; `tools/godot_smoke/battle_text_printer_smoke.gd` verifies slow/instant player-speed metadata, source `speed - 1` delay setup, speed-zero synchronous text, first-frame reveal, one A/B speed-up event, one display-text page wait, one source-text `\p` page wait, source-text `{PAUSE}` metadata, source-text `\l` plus trailing terminator handling, a direct source-byte newline event count of 3 independent of source-text escapes, source-byte prompt-clear and prompt-scroll metadata, EXT_CTRL_CODE_PAUSE byte args, metadata-only PLAY_SE/WAIT_SE events, and link speed override metadata; `tools/godot_smoke/battle_window_renderer_smoke.gd` verifies action/menu and move/menu 240x160 layer composition, generated source/composite/text rects, opaque RGBA pixels, immediate menu text, explicit asynchronous message reveal, one renderer-level display page wait, one renderer-level source-record prompt-clear wait from generated bytes with 3 source-byte events, and no runtime palette/source-color keys; `tools/godot_smoke/battle_scene_smoke.gd` verifies `BattleScene` uses the renderer/generated text info, passes generated source bytes for the static PP label with 2 source-byte events, and keeps dynamic action prompt/type text on source-text event streams.
