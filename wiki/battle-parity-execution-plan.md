...
- [ ] B8.2 Implement source window layer renderer.
  ...
  - Remaining for this task: source-capture screenshot comparison against the new Godot-side action/message/move signatures, real link synchronization/recorded action playback/full battle controller flow beyond setup metadata and text-window propagation, and full `RenderText` pixel equivalence beyond the current first-pass control-code side-effect coverage.
  ...
  - Validate next: compare the Godot-side 240x160 action/message/move signatures against source captures, extend link/recorded coverage into real link synchronization/recorded action playback/full battle controller flow beyond setup metadata and text-window propagation, and continue exact RenderText pixel equivalence for remaining control/font/string paths.
