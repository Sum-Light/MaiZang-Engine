...
- [ ] B8.2 Implement source window layer renderer.
  ...
  - Current generated/runtime metrics: 10 source-window composite rects are generated from `sStandardBattleWindowTemplates`; 25 normal battle-window `text_info` records are generated from `sTextOnWindowsInfo_Normal`; `text_printer` metadata preserves `BattlePutTextOnWindow` message speed source, A/B speed-up flag intent, recorded-battle speeds `[8,4,1,0]`, and player text speed delay/modifier/scroll tables. Runtime `BattleTextPrinter` resolves `GetPlayerTextSpeedDelay`, models the source `AddTextPrinter` nonzero-speed `speed - 1` delay counter, supports speed-zero synchronous menu text, can parse generated `source_text` records for `\n`, `\l`, `\p`, trailing `$`, `{PAUSE n}`, `{PAUSE_UNTIL_PRESS}`, metadata-only audio tokens such as `{PLAY_SE ...}`/`{WAIT_SE}`, and style controls, and still falls back to display-text events when no source text is available. It also exposes first-pass A/B delay clearing, per-window source-text labels/control counts, renderer `show_message_record` propagation, and link/recorded speed override/auto-scroll metadata.
  - Remaining for this task: exact source glyph renderer, encoded source byte-stream parsing rather than source-text string parsing, full `RenderText` control-code pixel/layout side effects, exact scroll/clear window behavior, full link/recorded battle context, and screenshot comparison against source captures.
  - Current validation: `tools/godot_smoke/data_registry_battle_interface_smoke.gd` verifies the 10 generated composite rects plus 25 generated text-info records and text-printer metadata; `tools/godot_smoke/battle_text_printer_smoke.gd` verifies slow/instant player-speed metadata, source `speed - 1` delay setup, speed-zero synchronous text, first-frame reveal, one A/B speed-up event, one display-text page wait, one source-text `\p` page wait, source-text `{PAUSE}` metadata, source-text `\l` plus trailing terminator handling, metadata-only PLAY_SE/WAIT_SE events, and link speed override metadata; `tools/godot_smoke/battle_window_renderer_smoke.gd` verifies action/menu and move/menu 240x160 layer composition, generated source/composite/text rects, opaque RGBA pixels, immediate menu text, explicit asynchronous message reveal, one renderer-level display page wait, one renderer-level source-record page wait, and no runtime palette/source-color keys; `tools/godot_smoke/battle_scene_smoke.gd` verifies `BattleScene` uses the renderer/generated text info and passes source-text event streams for action prompt, PP label, and move type snapshots.
