...
- [ ] B8.2 Implement source window layer renderer.
  ...
  - Current generated metrics: 10 source-window composite rects are generated from `sStandardBattleWindowTemplates` into `interface.json`; the renderer no longer owns per-window composite-row overrides.
  - Remaining for this task: exact source glyph renderer, `BattlePutTextOnWindow` printer cadence, text speed/wait playback, and screenshot comparison against source captures.
  - Current validation: `tools/godot_smoke/data_registry_battle_interface_smoke.gd` verifies the 10 generated composite rects; `tools/godot_smoke/battle_window_renderer_smoke.gd` verifies action/menu and move/menu 240x160 layer composition, generated source/composite rects, opaque RGBA pixels, and no runtime palette/source-color keys; `tools/godot_smoke/battle_scene_smoke.gd` verifies `BattleScene` uses the renderer for intro/action/move snapshots.
