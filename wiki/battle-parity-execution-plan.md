...
- [ ] B8.2 Implement source window layer renderer.
  ...
  - Remaining for this task: exact transparent source-capture acquisition/import for the 3 expected action/message/move PNGs under `assets/source/battle_window_captures/` now has `tools/importer/import_battle_window_captures.py` as the guarded import path; real link synchronization/recorded action playback/full battle controller flow beyond setup metadata and text-window propagation, and full `RenderText` pixel equivalence beyond the current first-pass control-code side-effect coverage also remain. The importer accepts only 240x160 non-interlaced RGBA transparent window-layer captures named `action.png`, `message.png`, and `move.png`, and rejects opaque/full-scene screenshots. The comparison gate still reports 3 expected captures, 3 missing captures, 0 compared captures, and 0 source-capture pixel diffs while real captures are absent; the 24 full-scene profile screenshots are available for layout work but are not used as exact window-layer captures.
