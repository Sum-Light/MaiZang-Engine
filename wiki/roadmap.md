...
- Trigger coordinate events after player movement. First pass done for normal `var`/`var_value` coord triggers through `MapRuntime` and `EventManager`; first-pass standard wild encounter dispatch now runs after generated coord/current-cell warp checks. Full weather, misc walking scripts, step-count, Repel/DexNav step handling, and forced-movement chains remain future work.
