...
- Add source-visible transition presentation. First structured sequence slice done: `EventManager` emits normal/silent and door transition sequences with traced source step order, 16-frame door/player timings, and source-derived destination exit-task selection from metatile behavior names; `Main` consumes them with a black overlay plus input lock. Future work must turn this contract into real door metatile animation, player step-in/out animation, fade/black-screen ordering, audio cues, exact destination reveal timing, and destination exit-task animation playback.
