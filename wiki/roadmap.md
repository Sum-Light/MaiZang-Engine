...
- Support a minimal ScriptVM command set:
  ...
  - `setmetatile`. First pass done after tracing `ScrCmd_setmetatile`, map-grid masks, and `MapGridSetMetatileIdAt`: `ScriptVM` resolves generated source metatile labels and `MapRuntime` applies the current-map mutation while preserving elevation. Generated map-script lifecycle hooks such as automatic `MAP_SCRIPT_ON_LOAD` dispatch remain future work.
