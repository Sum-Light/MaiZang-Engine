...
- Support a minimal ScriptVM command set:
  ...
  - `special`. First pass done for the first-slice string placeholder functions `GetPlayerBigGuyGirlString` and `GetRivalSonDaughterString`, writing `STR_VAR_1` and expanding `{PLAYER}`, `{KUN}`, `{RIVAL}`, and `{STR_VAR_1/2/3}` in runtime messages while preserving unexpanded text, source placeholder ids, substitution metadata, and runtime string vars. Broader `gSpecials`, `specialvar`, version/team placeholders, and other placeholders remain future work.
