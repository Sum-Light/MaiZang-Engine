...
- Add source-visible transition presentation. First structured sequence slice done: `EventManager` emits normal/silent and door transition sequences with traced source step order and 16-frame door/player timings, and `Main` consumes them with a black overlay plus input lock. Future work must turn this contract into real door metatile animation, player step-in/out animation, fade/black-screen ordering, audio cues, exact destination reveal timing, and destination exit-task branching.
