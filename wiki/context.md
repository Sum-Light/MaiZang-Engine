...
- `scenes/main.tscn` includes a debug dialogue panel driven by `EventManager`; object/sign interactions and first-pass coordinate triggers now execute the first generated script slice through `ScriptVM`, apply dispatch-time runtime effects, and show emitted dialogue text, while warps remain placeholders.
...
- For gameplay features as well as graphics imports, preserve the source game's visible rules and outcomes while dropping GBA hardware/resource constraints from the Godot runtime. Palette slots, tile/palette memory limits, binary metatile/map packing, and similar representation details should be decoded into normal Godot assets/data unless a gameplay rule specifically depends on them.
