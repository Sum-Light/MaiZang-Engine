...
- Shared `PlayersHouse_1F` scripts from source `data/scripts/players_house.inc` are not exported yet; Brendan/May house OnFrame intro scripts currently run their source `setvar` setup and then stop with `missing_branch_target` at the shared `PlayersHouse_1F_EventScript_EnterHouseMovingIn` target.
...
- `ScriptVM` now resolves numeric `LOCALID_*` script operands from the current generated map's object-event order (`tools/mapjson/mapjson.cpp` emits local-id constants as object index + 1) plus source special ids such as `LOCALID_PLAYER`, so commands like Brendan/May house Mom setup write source-compatible values.
