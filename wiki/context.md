...
- `ScriptVM` now parses the first runtime text-control slice after placeholder expansion: `{COLOR ...}`, `{SHADOW ...}`, `{FONT_NORMAL}`, `{FONT_MALE}`, `{FONT_FEMALE}`, `{PAUSE n}`, and `{PAUSE_UNTIL_PRESS}` are removed from visible message `text` and preserved as `text_controls` metadata with source `EXT_CTRL_CODE_*` ids, source lengths, values, offsets, and wait intent. Unknown dynamic tokens such as `{B_PC_CREATOR_NAME}` remain visible until their source battle/message expansion path is traced.
