...
- Next executable task: continue B8.2 source battle window layer rendering after generated composite rect and text-info metadata migration; exact source glyph rendering, real `BattlePutTextOnWindow`/`AddTextPrinter` cadence playback, text wait/page/down-arrow behavior, and screenshot comparison remain before B8.2 can be marked complete. B3 battle script VM and B13.4/B13.5 selector polish remain follow-up lanes.
