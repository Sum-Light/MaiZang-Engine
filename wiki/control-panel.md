...
- Section 5 is now 10/12 complete: layer-aware normal/covered/split rendering, top overlay drawing, source subpriority depth ordering, presentation-only layer debug views, first-pass `setmetatile` layer redraw cache updates, and first-pass border/connection layer rendering are implemented for the first-slice maps.
- Next executable task: keep source collision/elevation queries independent from presentation-only layer toggles.
...
- Continue `wiki/overworld-parity-todo.md` section 5: keep source collision/elevation queries independent from presentation-only layer toggles.
- Current overworld Section 5 status: 10/12 complete; border and north connection cells now resolve through the same layer-aware bottom/middle/top atlas path as in-bounds cells, and L cycles a presentation-only layer debug view across all/bottom/middle/top without mutating gameplay map, collision, or elevation data. The legacy F8 binding is removed because it can conflict with the Godot/editor/window shortcut path and exit the running preview.
...
- Next executable overworld task: keep source collision/elevation queries independent from presentation-only layer toggles.
