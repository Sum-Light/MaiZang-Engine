...
- Next executable task: support independent primary and secondary animation counters.
...
- Continue `wiki/overworld-parity-todo.md` section 6: support independent primary and secondary animation counters.
...
- Current overworld Section 6 status: 9/14 complete; `tileset_header_report.json` now exports callback-to-map metadata for 785 layouts, 939 map source records, 137 layout tileset pairs, 136 referenced tilesets, 31 callback symbols, 174 generated RGBA tileset-animation frame strips, and source schedule/copy-target metadata from `src/tileset_anims.c`: 31 init functions, 25 active init functions, 59 scheduled events, 57 tile-copy events, 2 Battle Dome source-color blend metadata-only events, 41 tile-copy append records, and 58,376 affected metatile references. Runtime `TilesetAnimationPlayer` now initializes on map load through `MapRuntime.map_changed`: Littleroot creates 2 role states, with `gTileset_General` active for 5 schedule events/5 copy targets and `gTileset_Petalburg` init-only with `NULL` event callback. Playback, independent counters, transition pause/reset, and renderer tile updates remain pending; source palette/color data remains import metadata, not runtime logic.
...
- Next executable overworld task: support independent primary and secondary animation counters.
