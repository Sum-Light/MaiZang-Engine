...
- Section 7 is now 1/16 complete after expanding `field_door_trace.json` to parse all 85/85 `sDoorAnimGraphicsTable` resources: 53 active Emerald runtime-target rows and 32 FRLG metadata-only rows.
- Runtime animation/gameplay logic still has 0 `palette`/`source_color`/`source_palette` references under `scripts/` and `scenes/`; source palette selector arrays remain importer metadata.
- Next executable overworld task: export small door, big door, FRLG door, sliding door, arena door, and expansion-specific variants from the parsed door resource table.
...
- Continue `wiki/overworld-parity-todo.md` section 7: export small, big, FRLG, sliding, arena, and expansion-specific door variants from the parsed `sDoorAnimGraphicsTable` resources.
...
- Current overworld Section 7 status: 1/16 complete; `field_door_trace.json` now expands all 85/85 `sDoorAnimGraphicsTable` rows into source resource metadata, with 53 active Emerald runtime-target rows, 32 FRLG metadata-only rows, 85/85 tile resources resolved, 85/85 palette selector arrays resolved, and 85/85 metatile ids resolved. Runtime export/lookup for all parsed resources remains first-pass pending.
