...
- Next executable task: implement a runtime `TilesetAnimationPlayer` that initializes on map load.
...
- Continue `wiki/overworld-parity-todo.md` section 6: implement a runtime `TilesetAnimationPlayer` that initializes on map load.
...
- Current overworld Section 6 status: 8/14 complete; `tileset_header_report.json` now exports callback-to-map metadata for 785 layouts, 939 map source records, 137 layout tileset pairs, 136 referenced tilesets, and 31 callback symbols, plus 174 generated RGBA tileset-animation frame strips from 182 editable source PNG refs under `assets/generated/tileset_anims/`. It also exports source schedule/copy-target metadata from `src/tileset_anims.c`: 31 init functions, 25 active init functions, 59 scheduled events, 57 tile-copy events, 2 Battle Dome source-color blend metadata-only events, 41 tile-copy append records, 41 appends with affected metatile ids, and 58,376 affected metatile references. `InitTilesetAnim_General` maps to 238 layouts/227 map records and carries 5 `TilesetAnim_General` tile-copy events; `InitTilesetAnim_Petalburg` maps to 6/6 including `MAP_LITTLEROOT_TOWN` and is correctly init-only with no tile-copy callback; runtime tileset animation playback remains pending.
...
- Next executable overworld task: implement a runtime `TilesetAnimationPlayer` that initializes on map load.
