...
- Next executable task: export animation image sources and generated RGBA frame strips.
...
- Continue `wiki/battle-parity-execution-plan.md` B8.2: broaden `RenderText` control-code pixel side effects beyond the first-pass clear/scroll slice, extend link/recorded coverage beyond the first-pass text-printer context into full battle controller context, and compare action/message/move windows against source captures.
- Continue `wiki/overworld-parity-todo.md` section 6: export animation image sources and generated RGBA frame strips.
...
- Current overworld Section 6 status: 6/14 complete; `tileset_header_report.json` now exports callback-to-map metadata for 785 layouts, 939 map source records, 137 layout tileset pairs, 136 referenced tilesets, and 31 callback symbols. `InitTilesetAnim_General` maps to 238 layouts/227 map records and `InitTilesetAnim_Petalburg` maps to 6/6, both including `MAP_LITTLEROOT_TOWN`; runtime tileset animation playback remains pending.
