...
- Next executable task: continue B8.2 source battle window layer rendering after the first-pass plain visible-string `BattleTextPrinter` cadence slice; exact source glyph rendering, source byte-stream/control-code `RenderText` handling, text wait/page/down-arrow behavior, A/B acceleration, link/recorded speed overrides, and screenshot comparison remain before B8.2 can be marked complete. B3 battle script VM and B13.4/B13.5 selector polish remain follow-up lanes.
