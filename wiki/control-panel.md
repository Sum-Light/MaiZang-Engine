...
- Next executable task: continue B8.2 source battle window layer rendering after the first-pass `BattleWindowRenderer`; source glyph/text-printer timing, per-window composite metadata, and screenshot comparison remain before B8.2 can be marked complete. B3 battle script VM and B13.4/B13.5 selector polish remain follow-up lanes.
