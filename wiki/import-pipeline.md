...
`tools/importer/export_tilesets.py` exports one map's primary/secondary tileset pair into source-color-baked tileset metadata, a flattened RGBA debug metatile atlas, and bottom/middle/top RGBA layer atlases. The flattened atlas is a temporary debug-only artifact and is explicitly marked non-equivalent for runtime source-layer rendering. It accepts `--config`, `--source`, `--map`, `--output-data-root`, and `--output-asset-root`.
...
- Builds separate bottom/middle/top RGBA layer atlases for source-traced `METATILE_LAYER_TYPE_NORMAL`, `METATILE_LAYER_TYPE_COVERED`, and `METATILE_LAYER_TYPE_SPLIT` placement. Normal records keep the source BG3 fill tile `0x3014` explicit; covered and split records map source tile slots to the BG3/BG2/BG1-equivalent render roles.
...
- Writes `assets/generated/tilesets/*_layer_bottom.png`, `*_layer_middle.png`, and `*_layer_top.png` for exported first-slice tilesets.
- Writes `data/generated/tilesets/littleroot_town.json` with atlas metadata, layer-rendering metadata, source tile entries, per-metatile `render_layers`, metatile attributes, metatile behavior names, metatile label ids, used metatile ids, coverage notes, and warnings.
